...
import 'package:mobile/screens/bottom_navigation/home/controllers/home_controller.dart';
...
class _MyAlumniInfoViweScreenState extends State<MyAlumniInfoViweScreen> {
  ...
  void getUserPerson() {
    ...
    _homeController.alumniPersonModel.value =
        campusAppsPortalInstance.getAlumniUserPerson();
    _homeController.userPersonModel.value = campusAppsPortalInstance.getUserPerson();
    ...
    //   UserPerson = user;
    // });
    if(_homeController.userPersonModel.value.mailing_address?.id == null){

    selectedDistrictId =  _homeController.userPersonModel.value.mailing_address?.city?.district?.id??0;
    selectedCityId =_homeController
          .userPersonModel.value.mailing_address?.city?.id??0;
    }else{
      selectedDistrictId = _homeController
          .alumniPersonModel.value.mailing_address?.city?.district?.id;
      selectedCityId = _homeController
          .alumniPersonModel.value.mailing_address?.city?.id ?? 0;
      // selectedDistrictId = AlumniUser.mailing_address?.city?.district?.id;
      // selectedCityId = AlumniUser.mailing_address?.city?.id ?? 0;
    }
    // print("==mailing_address?.id==${AlumniUser.mailing_address?.id}");
    // print("==City?.id==${AlumniUser.mailing_address?.city?.id}");
    // print("====Dis===:${AlumniUser.mailing_address?.city?.district?.id}");
    // print("====City===:${selectedCityId}");
      ...
      try {
        ...
        if (profilePictureUploadResponse.id != 0) {
          ...
          setState(() {
           ...
           AlumniUserPerson.alumni_profile_picture!.picture = base64Encode(_selectedProfilePictureImage!).toString();
          });
            ...
            children: [
              ...
              Column(
                ...
                children: [
                  ...
                  FutureBuilder<List<District>>(
                      ...
                      builder: (context, snapshot) {
                        if (snapshot.connectionState ==
                          ...
                          int? districtId = getDistrictIdByCityId(selectedCityId, districts);
                          ;
                  ...
                  TimelineWidget(
                    workTimeline:
                        AlumniUserPerson.alumni_work_experience != null
            ...
            child: DropdownButtonFormField<int>(
              ...
              onChanged: (value) async {
                ...
                setState(() {
                  ...
                  if (AlumniUserPerson.mailing_address == null && AlumniUserPerson.mailing_address!.city == null) {
  ...
  void _submitAlumniData(
    ...
    try {
      ...
      AlumniUserPerson  = await updateAlumniPerson(person2, id, selectedDistrictId);
      } else {
      AlumniUserPerson  = await createAlumniPerson(person2, selectedDistrictId);
      }
