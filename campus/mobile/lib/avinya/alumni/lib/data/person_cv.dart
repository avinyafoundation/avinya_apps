...
class PersonCv {
  ...
  factory PersonCv.fromJson(Map<String, dynamic> json) {
    return PersonCv(
        id: json['id'],
  ...
  Map<String, dynamic> toJson() => {
        ...
        if (drive_file_id != null) 'drive_file_id':drive_file_id,
...
Future<PersonCv> uploadCv(
    Uint8List? fileBytes, PersonCv cvMetaData,int personId) async {
  try {
    // Create the multipart request
    final uri = Uri.parse('${AppConfig.campusAlumniBffApiUrl}/alumni/$personId/cv');
    ...
    // Add the cv metadata details as a JSON string
    request.fields['cv_metadata'] = jsonEncode(cvMetaData);

    // Determine the MIME type of the file
    final mimeType = lookupMimeType('', headerBytes: fileBytes) ??
        'application/octet-stream';

    // Attach the cv to the request
    if (fileBytes != null) {
      request.files.add(
        http.MultipartFile(
          'cv_file', // Key for the file in form-data
    ...
    if (response.statusCode == 201) {
      ...
      final CvPostObject = PersonCv.fromJson(decoded);
