...
Future<List<EvaluationCriteria>> fetchEvaluationCriterias() async {
  final response = await http.get(
    Uri.parse(
        '${AppConfig.campusAssetsBffApiUrl}/student_vacancies/evaluation_criterias'),
...
Future<EvaluationCriteria> fetchEvaluationCriteria(String id) async {
  final response = await http.get(
    Uri.parse(
        '${AppConfig.campusAssetsBffApiUrl}/evaluation_criteria/evaluation_criterias/$id'),
    headers: <String, String>{
      'Content-Type': 'application/json; charset=UTF-8',
      'accept': 'application/json',
      'Authorization': 'Bearer ${AppConfig.campusBffApiKey}',
...
Future<http.Response> createEvaluationCriteria(
  ...
  final response = await http.post(
    Uri.parse(
        '${AppConfig.campusAssetsBffApiUrl}/evaluation_criteria/evaluation_criterias'),
    headers: <String, String>{
      'Content-Type': 'application/json; charset=UTF-8',
      'Authorization': 'Bearer ${AppConfig.campusBffApiKey}',
    },
