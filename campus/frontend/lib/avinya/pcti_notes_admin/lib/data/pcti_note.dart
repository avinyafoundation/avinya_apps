...
Future<List<ActivityInstance>> fetchActivityInstancesFuture(
  ...
  final response = await http.get(
    Uri.parse(
        '${AppConfig.campusPctiNotesBffApiUrl}/activity_instances_future?activity_id=$activity_id'),
    headers: <String, String>{
      'Content-Type': 'application/json; charset=UTF-8',
      'accept': 'application/json',
      'Authorization': 'Bearer ${AppConfig.campusBffApiKey}',
...
Future<List<Person>> fetchAvailableTeachers(int activity_instance_id) async {
  final response = await http.get(
    Uri.parse(
        '${AppConfig.campusPctiNotesBffApiUrl}/available_teachers?activity_instance_id=$activity_instance_id'),
    headers: <String, String>{
      'Content-Type': 'application/json; charset=UTF-8',
      'accept': 'application/json',
      'Authorization': 'Bearer ${AppConfig.campusBffApiKey}',
