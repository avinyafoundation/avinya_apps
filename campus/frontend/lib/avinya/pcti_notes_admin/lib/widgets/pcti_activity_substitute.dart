import 'package:pcti_notes_admin/data/activity_particpant.dart';
...
class _selectPctiSubstituteState extends State<selectPctiSubstitute> {
  ...
  void initState() {
    super.initState();
    futurePctiActivities =
        fetchPctiActivities(); // for now get notes for activity id 1
  }
                        ...
                        ElevatedButton(
                          ...
                          onPressed: () {
                            Navigator.of(context)
                              ...
                              builder: (context) => selectActivityInstance(
                                  pctiActivity: _selectedActivity!),
                            ));
                          },
                        ),
                      ],
...
class selectActivityInstance extends StatefulWidget {
  ...
  @override
  State<selectActivityInstance> createState() =>
...
class _selectActivityInstanceState extends State<selectActivityInstance> {
  ...
  void initState() {
    super.initState();
    futurePctiActivityInstances = fetchActivityInstancesFuture(
        pctiActivity!.id!); // for now get notes for activity id 1
  }

  Future<List<ActivityInstance>> refreshPctiActivityInstancesState() async {
    futurePctiActivityInstances =
        fetchActivityInstancesFuture(pctiActivity!.id!);
                          ...
                          ElevatedButton(
                            ...
                            onPressed: () {
                              setState(() {
                                ...
                                Navigator.of(context)
                                    .push<void>(MaterialPageRoute<void>(
                                  builder: (context) => selectAvailableTeacher(
                                      pctiActivityInstance:
                                          _selectedActivityInstance!),
                                ));
                              });
                            },
                          ),
...
class selectAvailableTeacher extends StatefulWidget {
  ...
  @override
  State<selectAvailableTeacher> createState() =>
...
class _selectAvailableTeacherState extends State<selectAvailableTeacher> {
  ...
  Future<void> _addTeacherParticipant(BuildContext context,
      ActivityInstance selectedActivityInstance, Person selectedTeacher) async {
    try {
