...
import 'package:feedbacks/data/activity_instance.dart';
...
Future<List<Activity>> fetchActivitys() async {
  final response = await http
      .get(Uri.parse(AppConfig.campusConfigBffApiUrl + '/activity'));
...
Future<Activity> fetchActivity(String name) async {
  final response = await http
      .get(Uri.parse(AppConfig.campusConfigBffApiUrl + '/activity/$name'));
...
Future<http.Response> createActivity(Activity activity) async {
  final response = await http.post(
    Uri.parse(AppConfig.campusConfigBffApiUrl + '/activity'),
...
Future<http.Response> deleteActivity(String id) async {
  final http.Response response = await http.delete(
    Uri.parse(AppConfig.campusConfigBffApiUrl + '/activity/$id'),
    headers: <String, String>{
      'Content-Type': 'application/json; charset=UTF-8',
    },
  );
