...
public isolated client class GraphqlClient {
    ...
    remote isolated function getPerson(string id) returns GetPersonResponse|graphql:ClientError {
        string query = string `query getPerson($id:String!) {person_by_digital_id(id:$id) {id preferred_name full_name date_of_birth sex asgardeo_id jwt_sub_id jwt_email permanent_address {city {id} street_address phone id} mailing_address {city {id} street_address phone id} phone organization {id description notes address {id} avinya_type {id} phone name {name_en name_si name_ta} child_organizations {id} parent_organizations {id} people {id} vacancies {id}} avinya_type {id active global_type name foundation_type focus level description} avinya_type_id notes nic_no passport_no id_no email child_students {id preferred_name full_name date_of_birth sex asgardeo_id jwt_sub_id jwt_email permanent_address {id} mailing_address {id} phone organization {id} avinya_type {id} avinya_type_id notes nic_no passport_no id_no email child_students {id} parent_students {id} street_address digital_id avinya_phone bank_name bank_account_number bank_account_name} parent_students {id preferred_name full_name date_of_birth sex asgardeo_id jwt_sub_id jwt_email permanent_address {id} mailing_address {id} phone organization {id} avinya_type {id} avinya_type_id notes nic_no passport_no id_no email child_students {id} parent_students {id} street_address digital_id avinya_phone bank_name bank_account_number bank_account_name} street_address digital_id avinya_phone bank_name bank_account_number bank_account_name}}`;
